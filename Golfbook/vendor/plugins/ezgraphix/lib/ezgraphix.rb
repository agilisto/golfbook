   ...
   #
    class Graphic
     ...
     #Returns the Graphic's type.
      ...
      def to_xml
        multiple = render_options.delete(:multiple)
        options = parse_options(self.render_options)
        g_xml = Builder::XmlMarkup.new
        unless multiple
          color = ((render_options[:c_type] == 'line') ? COLORS.first : 'rand')
          escaped_xml = g_xml.graph(options) do
            self.data.each{ |k,v|
              g_xml.set :value => v, :name => k, :color => color
            }
          end
        else
          escaped_xml = g_xml.graph(options) do
            g_xml.categories do
              self.categories.each do |cat|
                g_xml.category(:value => cat, :showName => 1, :name => cat)
              end
            end
            self.data.each do |dataseries|
              g_xml.dataset(dataseries[:options].merge(:color => self.rand_color)) do
                dataseries[:series].each do |value|
                  g_xml.set :value => value
                end
              end
            end
          end
